...
Read it as `total = free + (self = model + context + compute) + unaccounted`. The interesting numbers for tuning are model, context, and compute. Free is what's left for the OS and other apps.
...
Pin all layers to GPU with `999`. Lower values offload tail layers to CPU, which is slow. Only useful when the model doesn't fit at any ctx-size.
...
If you find yourself dropping `n-gpu-layers` below 999 to fit, take a quant level down (Q4_K_XL to Q3_K_M) instead. Partial CPU offload murders throughput.
